...
def attack_ownaim(i, t):
    # 弾道を自機狙いに
    # 改造：自機狙いの１つの大きい弾に複数の弾が精度の低い追従
    if t.heading() == 180:
        angle = deg_own_shot(t)
        t.seth(angle + i%3 * 3 * (-1) ** i)
...
def attack_3way(i, t):
    ...
    if i < 2:
        if t.heading() == 180:
            t.seth(deg_own_shot(t) - 20)
        t.forward(speed - i)
    elif i < 4:
        if t.heading() == 180:
            t.seth(deg_own_shot(t))
        t.forward(speed + 2 - i)
    else :
        if t.heading() == 180:
            t.seth(deg_own_shot(t) + 20)
...
def game():
    ...
    for i, t in enumerate(targets):
        ...
        if is_hit(t, my_turtle):
            ...
            screen.update()
            return
        elif score%10 == 0 and sc_disp != score:
            # scoreを追加
            time_text.clear()
            time_text.write(f"score: {int(score)}",
                            font=("helvetica", 24))
